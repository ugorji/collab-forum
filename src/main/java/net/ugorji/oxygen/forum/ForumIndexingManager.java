...
public class ForumIndexingManager extends MarkupIndexingManager {
  ...
  public ForumIndexingManager() throws Exception {
    ...
    // analyzer =
    // (Analyzer)Class.forName(engine.getProperty(ForumConstants.INDEXING_ANALYZER_CLASSNAME_KEY)).newInstance();
    analyzer = new MarkupAnalyzer(this);
    analyzerForSearching = new MarkupAnalyzer(this);
    supportFullTextSearch =
        "true".equals(engine.getProperty(OxygenConstants.FULL_TEXT_SEARCH_SUPPORTED_KEY));
  ...
  private void createDocuments(IndexWriter iwriter, Topic t) throws Exception {
    Long lastPostId =
        ForumLocal.getForumEngine().getForumStatisticsManager().getLastPostIdForTopic(t.getId());
    Post p = (Post) ForumLocal.getForumEngine().getForumDAO().get(Post.class, lastPostId, false);
    if (p != null) {
      Document doc = new Document();
      doc.add(
          new Field(
              ForumConstants.SEARCH_INDEX_TOPIC_AUTHOR,
              t.getAuthor().getName(),
              Field.Store.YES,
              Field.Index.UN_TOKENIZED));
      doc.add(
          new Field(
              ForumConstants.SEARCH_INDEX_TOPIC_LAST_POST_DATE,
              String.valueOf(p.getDate().getTime()),
              Field.Store.YES,
              Field.Index.UN_TOKENIZED));
      doc.add(
          new Field(
              ForumConstants.SEARCH_INDEX_TOPIC_ID,
              String.valueOf(t.getId()),
              Field.Store.YES,
              Field.Index.UN_TOKENIZED));
      doc.add(
          new Field(
              ForumConstants.SEARCH_INDEX_FORUM_ID,
              String.valueOf(t.getForum().getId()),
              Field.Store.YES,
              Field.Index.UN_TOKENIZED));
      ...
      for (Iterator itr = t.getAllposts().iterator(); itr.hasNext(); ) {
        ...
        doc.add(
            new Field(
                ForumConstants.SEARCH_INDEX_POSTER,
                p.getAuthor().getName(),
                Field.Store.YES,
                Field.Index.UN_TOKENIZED));
        if (supportFullTextSearch) {
          doc.add(
              new Field(
                  ForumConstants.SEARCH_INDEX_TOPIC_CONTENTS,
                  p.getDetails(),
                  Field.Store.YES,
        ...
        doc.add(
            new Field(
                ForumConstants.SEARCH_INDEX_TITLE,
                p.getTitle(),
                Field.Store.YES,
                Field.Index.TOKENIZED));
      }
  ...
  public void search(Map m, HitsHandler hhdlr) throws Exception {
    ...
    for (Iterator itr = m.keySet().iterator(); itr.hasNext(); ) {
      ...
      if (val instanceof String
          ...
          && (ForumConstants.SEARCH_INDEX_TOPIC_CONTENTS.equals(key)
              || ForumConstants.SEARCH_INDEX_POSTER.equals(key)
              || ForumConstants.SEARCH_INDEX_TITLE.equals(key)
              || ForumConstants.SEARCH_INDEX_FORUM_ID.equals(key))) {
        query.add(
            new BooleanClause(
                new QueryParser(key, analyzerForSearching).parse((String) val),
  ...
  protected MarkupIndexingParser newParser(String name, Reader r, List _tokenstrings, HashSet _hs)
    ...
    MarkupIndexingParser mip =
        new MarkupIndexingParser(
            fe.getMarkupParserFactory().newMarkupParser(r),
            rc,
            fre.getRenderEngine(),
            name,
            _tokenstrings,
